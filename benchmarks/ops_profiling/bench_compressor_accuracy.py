"""fused compressor vs split（MatMulV3 + compress_norm_rope）精度与稳定性对比测试。

用法：
    python bench_compressor_accuracy.py               # c4 全量（默认，含确定性/精度/padding 行分析）
    python bench_compressor_accuracy.py --ratio 128   # c128 全量精度与稳定性验证
...
3. split1 的 mm.chunk(2, -1) 返回非 contiguous 视图（stride=2*out_dim），用于
   验证生产路径的一次 fused F.linear + 零拷贝切片。compress_norm_rope 必须按输入 row stride
   读取；split2 保留两个独立 F.linear（天然 contiguous）作为兼容性对照。
...
def run_split1(t):
    """生产路径：提前合并权重，一次 F.linear 后直接传入两个非连续切片。"""
    mm = nn.functional.linear(t["x"], t["w_cat"])
    mm_kv, mm_score = mm.chunk(2, dim=-1)
    out_dim = RATIO_CONFIG[t["ratio"]][0] * HEAD_DIM
    assert mm_kv.stride() == (2 * out_dim, 1)
    assert mm_score.stride() == (2 * out_dim, 1)
    # A singleton leading dimension is considered contiguous by PyTorch even
    # when its (unobservable) row stride is wider than the logical row.
    if mm_kv.shape[0] > 1:
        assert not mm_kv.is_contiguous() and not mm_score.is_contiguous()
    out = torch.ops._C_ascend.compress_norm_rope(
...
def run_split2(t):
    """兼容性对照：两个独立 F.linear，输出天然 contiguous。"""
...
def run_decode_chain(B, kv_len, ratio, seed, steps, start_pos0, desc):
    ...
    for step in range(steps):
        ...
        o_f = torch.ops._C_ascend.compressor(x, wkv_w, wgate_w, state_f, ape, norm_w,
                                             ...
                                             start_pos=sp, **kw)
        o_e = torch.ops._C_ascend.compress_norm_rope(
...
def run_c128(M, kv_len, seed):
    """单独验证 c128 的 fused 与 split 实现，便于隔离 NPU kernel 错误。"""
    ...
    try:
        ...
        print(f"  compress_norm_rope FAIL: {str(e)[:80]}")
        return False

    print("  compress_norm_rope OK，进入精度对比:")
    t["state_cache"] = base.clone()
    ok = _check_determinism(t, "split2")
    ok &= _stats("cmp_kv", out_f, out_e, "fused vs split")
    ok &= _stats("cache", cache_f, cache_e, "cache split")
...
def run_c128_full():
    ...
    cases = [
        ...
        # decode：覆盖单 token 与 MTP 多 token，包含稀疏/全边界。
        (1, 1, 4096, 128, 15, 126, "c128 decode B=1 无边界"),
        (1, 1, 4096, 128, 16, 127, "c128 decode B=1 单边界"),
        (32, 32, 4096, 128, 17, None, "c128 decode B=32 单边界",
         [1] * 32, [127] + [128] * 31),
        (8, 8, 4096, 128, 18, 127, "c128 decode B=8 全边界"),
        (32, 8, 4096, 128, 19, None, "c128 MTP B=8 q=4 单边界",
         [4] * 8, [124] + [128] * 7),
        (32, 8, 4096, 128, 20, 128, "c128 MTP B=8 q=4 无边界"),
        (32, 8, 4096, 128, 21, 124, "c128 MTP B=8 q=4 全边界"),
    ]
    return cases
...
def main():
    ...
    parser.add_argument("--c128-diagnose", action="store_true",
                        help="只跑 c128 隔离诊断（fused vs compress_norm_rope）")
