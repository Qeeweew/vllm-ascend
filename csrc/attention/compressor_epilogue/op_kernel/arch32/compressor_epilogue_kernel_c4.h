...
/*!
 * \file compressor_epilogue_kernel_c4.h
...
#ifndef COMPRESSOR_EPILOGUE_KERNEL_C4_H
#define COMPRESSOR_EPILOGUE_KERNEL_C4_H
...
class CompressorEpilogueKernelC4 {
public:
...
private:
    ...
    {
        uint64_t srcOff = (uint64_t)g.xRow * rowLen_;
        DataCopy(kvStage, mmKvGm_[srcOff], g.nTok * rowLen_);
        DataCopy(scoreStage, mmScoreGm_[srcOff], g.nTok * rowLen_);
    }
                                          ...
                                          const LocalTensor<float> &apeUb)
    {
        ...
        DataCopyAlignGmToUb(kvLeft, mmKvGm_[(uint64_t)xRowL * rowLen_], rowCnt, headDim_, rowLen_, headDim_);
        DataCopyAlignGmToUb(scoreLeft, mmScoreGm_[(uint64_t)xRowL * rowLen_], rowCnt, headDim_, rowLen_, headDim_);
        SetFlag<HardEvent::MTE2_V>(evtMte2ToVWin_);
        WaitFlag<HardEvent::MTE2_V>(evtMte2ToVWin_);
...
#endif // COMPRESSOR_EPILOGUE_KERNEL_C4_H
