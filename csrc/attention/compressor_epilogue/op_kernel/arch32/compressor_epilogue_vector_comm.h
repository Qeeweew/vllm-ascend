...
/*!
 * \file compressor_epilogue_vector_comm.h
 * \brief vector 公共组件：ColumnSoftMax/ColumnSum（移植自 attention/compressor，语义一致）
 */

#ifndef COMPRESSOR_EPILOGUE_VECTOR_COMM_H
#define COMPRESSOR_EPILOGUE_VECTOR_COMM_H
...
// 矩阵逐行乘向量（移植自 attention/compressor，MatRpeatParam → MatRepeatParam）
...
// 矩阵每行求和（移植自 attention/compressor；契约：srcLocal 与 shareTmpUb 必须别名同一块空间）
...
// 矩阵每行除以对应元素（移植自 attention/compressor；src1Local 需扩展到 [row, FP32_BLOCK_ELEMENT_NUM]）
...
// RmsNorm（移植自 vllm-ascend rms_norm.h）：dst = src / sqrt(mean(src^2)+eps) * gamma
...
// SetGatherSrcOffset（移植自 vllm-ascend rope.h）：INTERLEAVE 模式的 Gather 偏移
...
// RotaryPosEmb（移植自 vllm-ascend rope.h）：对 [row, actualCol] 的 [baseAddr, baseAddr+col) 段做 rope
...
#endif // COMPRESSOR_EPILOGUE_VECTOR_COMM_H
