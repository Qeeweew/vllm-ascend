 ...
 * \brief CompressorEpilogue arch22 tiling（两阶段重构版）
 *
 * 集成自 ops-transformer（G_W_E/ops-transformer compressor-epilogue 分支）：
...
// ops-transformer（ref）日志宏兼容层：vllm-ascend 无 OP_LOGE_FOR_INVALID_*_WITH_REASON，
...
ge::graphStatus ConvertContext(gert::TilingContext &context, CompressorEpilogueContext &c)
{
...
ge::graphStatus TilingCompressorEpilogue(gert::TilingContext *context)
{
    ...
    if (t == 0) {
        ...
        bool isBf16 = (c.mmKv.desc != nullptr && c.mmKv.desc->GetDataType() == ge::DT_BF16);
        uint8_t dtype = isBf16 ? 0 : 1;
        c.tilingKey = 0; // EMPTY_X：kernel 直接返回
    ...
    // ── tiling key / blockDim / schedule ──
    // TILING_KEY 运行时分发（rms_norm_dynamic_quant 风格）：1=C4(coff=2 overlap)，2=C128(coff=1)。
    // dtype 由编译变体（DTYPE_MM_KV 宏）区分，不参与 key。
    c.tilingKey = (coff == (int64_t)COFF_DISABLE) ? 2 : 1;
    context->SetTilingKey(c.tilingKey);
