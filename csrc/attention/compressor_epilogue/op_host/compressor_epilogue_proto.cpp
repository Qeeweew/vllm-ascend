...
struct CompressorEpilogueProtoShapeParam {
...
ge::graphStatus GetCompressorEpilogueShapeDim(const gert::InferShapeContext* context, CompressorEpilogueProtoShapeParam &shapeParam)
{
...
ge::graphStatus SetCompressorEpilogueShapeDim(const CompressorEpilogueProtoShapeParam &shapeParam, gert::InferShapeContext* context)
{
...
ge::graphStatus InferDataTypeCompressorEpilogue(gert::InferDataTypeContext* context)
{
    OP_CHECK_IF(context == nullptr, OPS_REPORT_VECTOR_INNER_ERR("CompressorEpilogue", "Context is nullptr."),
               return ge::GRAPH_FAILED);
    OPS_LOG_I(context->GetNodeName(), "Enter CompressorEpilogue inferDataType impl.");
...
ge::graphStatus InferShapeCompressorEpilogue(gert::InferShapeContext* context)
{
    OP_CHECK_IF(context == nullptr, OPS_REPORT_VECTOR_INNER_ERR("CompressorEpilogue", "Context is nullptr."),
               return ge::GRAPH_FAILED);
    OPS_LOG_I(context->GetNodeName(), "Enter CompressorEpilogue infershape impl.");

    CompressorEpilogueProtoShapeParam shapeParam {};
    auto apiRet = GetCompressorEpilogueShapeDim(context, shapeParam);
    OPS_LOG_E_IF((apiRet != GRAPH_SUCCESS), context, return ge::GRAPH_FAILED, "Context get input shape failed");

    apiRet = SetCompressorEpilogueShapeDim(shapeParam, context);
    OPS_LOG_E_IF((apiRet != GRAPH_SUCCESS), context, return ge::GRAPH_FAILED, "Context set output shape failed");

    return GRAPH_SUCCESS;
}

IMPL_OP_INFERSHAPE(CompressorEpilogue).InferShape(InferShapeCompressorEpilogue).InferDataType(InferDataTypeCompressorEpilogue);
}  // namespace ops