 ...
 */
#ifndef MOE_GATING_TOP_K_HASH_WITHOUT_GROUP_BATCH_H
#define MOE_GATING_TOP_K_HASH_WITHOUT_GROUP_BATCH_H
...
{
    int64_t rp = rowBatch_ * paddedCount_;
    //    fp16 -inf = 0xFC00，bf16/fp32 -inf = 0xFF80
...
__aicore__ inline void MoeGatingTopKWithoutGroupBatch<T>::ComputeX(int64_t bufIdx, int64_t rows)
{
    ...
    if (normType_ == 1) {
        ...
        Exp(norm, xF, total);
        PipeBarrier<PIPE_V>();
        Adds(norm, norm, 1.0f, total);
        PipeBarrier<PIPE_V>();
        Ln(norm, norm, total);
        PipeBarrier<PIPE_V>();
        Sqrt(norm, norm, total);
        PipeBarrier<PIPE_V>();
    } else if (normType_ == 0) {
...
#endif // MOE_GATING_TOP_K_HASH_WITHOUT_GROUP_BATCH_H
