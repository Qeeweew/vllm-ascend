...
Ported from origin/deepseek-v4-w4a16-tp. Loading-only conversion shared by
CANN grouped matmul and the native decode kernel; scales are unchanged.
"""
    ...
    for k8 in range(begin, end):
        for n8_base in range(0, n_cols // 8, BLOCK_N8):
            ...
            for nibble in tl.static_range(8):
                n = n8 * 8 + nibble
                packed = tl.load(src + n * stride_src_n + k8 * stride_src_k8, mask=n < n_cols, other=0).to(tl.uint32)
                shift = nibble * 4
...
def repack_int4_moe(weight: torch.Tensor) -> torch.Tensor:
    ...
    if experts == 0 or n_cols == 0 or n_cols % 8 or k8 == 0:
        raise ValueError(f"INT4 repack requires positive dimensions and N divisible by 8, got {tuple(weight.shape)}")
