...
"""Hardware-neutral V4.1 multimodal components, with no registration side effects.

Adapted from vLLM's deepseek_v41/common/mm_preprocess.py. Keeping this small
CPU module in the plugin avoids importing the upstream model package, whose
__init__ selects NVIDIA/AMD kernels. This module never patches a registry.
...
# like the reference. V4.1 uses one reserved ID; processor-owned ranges
# distinguish actual image spans from literal/generated occurrences.
IMAGE_SENTINEL_BASE_ID = 129264


def image_sentinel_mask(token_ids: torch.Tensor) -> torch.Tensor:
    """Recognize the reserved ID; actual span masks also require MM metadata."""
...
def validate_image_sentinel_ids(tokenizer) -> None:
    ...
    if image_id != IMAGE_SENTINEL_BASE_ID:
        raise ValueError(
            ...
            "image_token_id); DeepSeek-V4.1 image spans use this reserved ID."
        )
...
class DeepseekV41VLProcessingInfo(BaseProcessingInfo):
    ...
    def get_image_size_with_most_features(self) -> ImageSize:
        ...
        # Match the release's uncapped worst case; retain the upstream
        # bounded square fallback for aspect-capped configurations.
        budget = hf_config.vision_max_n_token
        if hf_config.vision_max_wh_ratio is None:
            # With no aspect cap, one LLM row minimizes newline overhead and
            # maximizes both actual image features and ViT patches. A square
            # underestimates the release config's worst-case patch count.
            cell = patch_size * downsample_ratio
            return ImageSize(width=(budget - 3) * cell, height=cell)
        side = budget * patch_size * downsample_ratio
        best_h, best_w = solve_resize_ratio(side, side, patch_size, downsample_ratio, budget)
...
class DeepseekV41VLMultiModalProcessor(BaseMultiModalProcessor[DeepseekV41VLProcessingInfo]):
    def apply(self, inputs: ProcessorInputs, timing_ctx: TimingContext) -> MultiModalInput:
        ...
        placeholder_count = inputs.prompt.count(IMAGE_SENTINEL_BASE_ID)
        # Without image payloads a reserved token is ordinary prompt text.
        # Processor-owned MM ranges, not raw ID equality, define image spans.
        if image_count and placeholder_count != image_count:
            raise ValueError(f"Found {placeholder_count} image tokens but got {image_count} images")
        if image_count and self.info.get_hf_config().vision_n_layers == 0:
