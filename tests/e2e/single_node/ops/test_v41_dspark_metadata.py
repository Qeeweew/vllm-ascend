...
def make_inputs(batch, span, ragged=False):
    query_counts = [0 if ragged and request % 3 == 0 else 5 for request in range(batch)]
...
@pytest.mark.parametrize("batch", [1, 2, 4, 8, 16, 32])
def test_changed_graph_captures_schedule_and_attention(runtime, batch):
    ops, build_indices = runtime
    host = make_case(0, [5] * batch, [256] * batch)
    ...
    for replay in range(16):
        # Reject counts 0..5 cross the max-context boundary; repeated/empty
        # request slots and different active T reuse the same captured buffers.
        counts = [0 if (r + replay) % 4 == 0 else 5 for r in range(batch)]
        lengths = [256 - replay % 6 + count if count else 0 for count in counts]
        ...
        expected, expected_lse = oracle(host, cpu_indices, cpu_spans)
        check(actual, expected)
        torch.testing.assert_close(lse.cpu(), expected_lse, atol=0.015, rtol=0.003)
